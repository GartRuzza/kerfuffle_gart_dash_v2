...
**Build note (2026-08-31).** At build time the owner attached a **FantasyPros power-rankings dashboard** as the visual target, which added scope beyond the written issue. Four sub-decisions were confirmed up front, all consistent with the above: (a) **five-component emulation** — the team table plus, per selected team, Positional Rankings, Starter Rankings, a starters-vs-bench radar, and a Starting-Lineup chart (master–detail); (b) **the 0–100 score is shown alongside the raw Kerf points**, not instead of them; (c) **DST omitted from every chart** (extends decision 5 — not just totals); (d) **dynasty ships as a visibly greyed/disabled toggle** rather than being absent, so the deferral is legible. No reversal of D-20 — these are how the confirmed scope was rendered.

**Refinement note (2026-09-01).** After review, the owner asked the charts to show **actual value relative to the league**, not just rank. Confirmed choices (still no reversal of D-20): bars scale **value-relative** (league-worst → empty, best → full) rather than to the absolute max or in even rank-steps — this fixes the "everything looks like #1" clustering while staying honest about real gaps; a **league-median** reference + **hover popups** carry the numbers (nothing printed on the bars); the radar gets a **Starters/Bench/Both** pill with a **per-axis league-relative scale** (each axis min→center, max→edge — a deliberate relative-to-league read, the usual cross-axis caveat mitigated by the median reference); **still hand-rolled SVG, no charting dependency.** Bench axes use each team's **average** bench value per position.
