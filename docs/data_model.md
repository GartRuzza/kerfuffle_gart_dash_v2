...
**Informed by the source-profiling spike (issue #11 — [`profiles/PROFILE.md`](profiles/PROFILE.md)):** the raw archive was profiled field-by-field into a committed, shape-only profile ([`profiles/`](profiles/)). Evidence that shapes the storage schema:

- **Dead-cap pseudo-rows & Practice Squad (the open decision's evidence).** Across all 12 rosters: **170 real players, 10 Practice-Squad players, 0 commissioner-added dead-cap pseudo-rows** in the profiled (pre-auction) snapshot. A **Practice-Squad player is an ordinary player row** (real CBS id) sitting in a `Practice` section — it differs from an active player only by section/status, and per the rules counts against the $500 cap but not active-roster limits. A **dead-cap pseudo-row, when present, is a roster row with a salary but no `playerpage` link** (no CBS id) — this is the reliable way to classify it. None existed to sample here, so the modelling decision (roadmap open decision #7) stays open, now with a confirmed detection rule.
- **Roster `Pos` is the lineup _slot_, not the player's position** (values include `FLEX`, `RB-WR-TE`). The true position comes from the `Players` cell — the schema must not store roster `Pos` as position.
- **Contract** observed as **{1, 2, 3}** this snapshot (domain {1–4} per the rules) — the ingest invariant below holds.
- **`scoring_rule` is real and parsed** into [`profiles/cbs_scoring_rules.json`](profiles/cbs_scoring_rules.json) (24 rules; flat / per-unit / tiered). ⚠ **CBS `/rules` is authoritative and can diverge from the constitution** (observed: defensive Int = 2 on CBS vs 3 in the constitution) — the schema stores the *parsed* value, never a hardcoded one.
- **Transactions** expose `Date · Team · Players · Effective(week)` — **no type column and no FAB bid-amount column**, and the log is **paginated**. The `transaction` entity's types must be derived from row content, and FAB amounts are recoverable via the resulting salary rather than an itemized bid. Several CBS sources are **JS-rendered/paginated** (`/players`, `/transactions`, `/draft/results`, `/scoring/live`) — ingestion for those needs the paged/JS route, not the first static snapshot.
