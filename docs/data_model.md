...
**Last updated:** 2026-08-26 · **Latest migration:** `db/migrations/007_valuation.sql` (the issue-#20 valuation layer — `replacement_level`, `valuation`, `price_curve`; `006` is the issue-#19 backtest isolation — `pull.kind`/`pull.season` + `latest_pull` re-scoped to `kind='current'`; `005` is the issue-#18 projection layer; `004` is the issue-#17 historical layer; `003` is the D-12 superflex display board; `002` is the issue-#12 review fixes; `001` is the D-10 normalized layer + the D-11 dead-cap/Practice-Squad decision)
...
**Read model (built):** the **`board`** SQL view — the **latest-captured** pull's roster snapshot (`latest_pull` orders by `captured_at` among **`kind='current'`** pulls only — **not** by ingest order, and never a `kind='backtest'` pull: re-ingesting an older run after a parser fix must never make stale rosters "current", and the backtest's historical pulls, though captured most recently, are excluded by the kind filter — migration 006) joined with player/team identity and the display boards, **union** free agents (ranked players on no roster). **Both branches filter to the positions this league rosters** (no kickers) so the display domain can't be violated from either side. Dead-cap rows are stored but are not board rows.
...
## Valuation layer (built — migration 007, issue [#20](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/20))

The second derived layer, written by `npm run engine` in the SAME run as the projections (it reads `projection`, so it runs after it). See [D-13](decision_log.md) (method) and [D-17](decision_log.md) (the build's finalized params). Turns each player's Kerf points into auction dollars. **No `owner_ceiling_override`** — the owner's Ceiling stays session-only this issue (persisted ceilings are the auction-prep lens, roadmap #10).

| Entity (built) | Grain / key | What it holds |
| --- | --- | --- |
| `replacement_level` | engine_run × pos — **UNIQUE (engine_run_id, pos)** | The "last-starter" baseline `baseline_n` per position (QB24/RB34/WR34/TE17/DST12 — computed from the 12-team lineup + a 40/40/20 FLEX split, SFLEX = 100% QB) and the projected `replacement_points` at that rank (the N-th best player's Kerf points; NULL if fewer than N exist). `method='last_starter'`. |
| `price_curve` | engine_run × basis × pos × rank — **UNIQUE (engine_run_id, basis, pos, pos_rank)** | The market price curve KNOTS — "what the Nth-most-expensive player at this position costs," fit from real salaries. `basis` = `in_season` (current roster salaries) or `pre_auction` (the 2025 KERFUFFLE salaries incl. since-dropped players — [contract_history]). Kept so the curve BASIS is inspectable (drillability). **TRUFFLE is not read** ([D-15](decision_log.md)). |
| `valuation` | player × engine_run — **UNIQUE (engine_run_id, `cbs_player_id`)** | DERIVED per-player dollars. Per player: `kerf_points`, the league `replacement_points` + `par_league` (floored at 0), **`kerf_value`** (league-generic ceiling = 1 + PAR × $/point); the Raccoons `roster_repl_points` + `par_roster` + **`roster_value`** (replace-your-starter — value above our own worst eligible starter, NULL if no roster); **`market_in_season`** + **`market_pre_auction`** (read off `price_curve` by the player's Kerf `pos_rank_used`); and `components_json` (the $/point, discretionary, budget, roster-size params) for drill-down. **Offense only** — DST is not priced (no projection to score). |

**Replacement + dollars (built — `tools/engine/valuation.mjs`):** last-starter baselines are `nTeams × (dedicated slots + FLEX share)`, the SFLEX slot counted 100% as a QB (the superflex effect — QB24 not QB12); replacement points = the projected points of the baseline-ranked player. **Marginal $/point** = (`$500 × 12` − `$1 × 19 rosterable spots × 12 teams`) ÷ Σ points-above-replacement; `kerf_value = 1 + PAR × $/point`. **Prices sum to the cap** — Σ(`kerf_value` − 1) over the priced pool equals the discretionary money, an internal-balance check `npm run engine` asserts and logs. All conversion params (budget, roster spots, $/point) are tunable constants, documented in the module.

**Roster-aware value (built):** the Raccoons' optimal superflex starting lineup is built greedily by Kerf points; a candidate at position P is valued above the WEAKEST current starter in a slot P is eligible for (QB→QB/SFLEX; RB/WR/TE→their slot/FLEX/SFLEX). So a stacked position raises the bar (values a new player DOWN) and a thin one lowers it (UP). Falls back to league replacement for a position the roster can't field. Same $/point as the league-generic ceiling, so the two dollar numbers are directly comparable.

**Market price (built):** per position, salaries are sorted descending into a "what the Nth-best costs" curve; a player's price is read off by his Kerf positional rank, flattening to the cheapest observed salary past the last knot. Two bases (current + 2025 salaries). Blank/zero salaries and non-offensive positions are dropped; a position with no salary data yields a null price.

## The two client-side data shapes (unchanged)

- **`Player` / `PlayerRow`** (`lib/types.ts`) — the flat shape the UI consumes, now produced **only** by `lib/data/`. The **Kerf rank/tier fields** (`kerfOvrRank`/`kerfPosRank`/`kerfOvrTier`/`kerfPosTier`) and `projPts` are filled from the latest `engine_run`'s `projection` rows (issue #18); the **dollar fields** (`kerfValue`, `rosterValue`, `marketPrice`, `marketPreAuction`) are now filled from the latest run's `valuation` rows (issue #20). All are `number | null` — null (render "—") for players the engine can't price (defenses, unprojected). The owner's editable `ceiling` seeds from `kerfValue` (rounded to a whole dollar), session-only. Do not grow tables from this shape — it is the read model's output, not a schema.
...
| **Applied** | `001_normalized_schema.sql` — the seven normalized entities + the `board`/`latest_pull` views (2026-08-25); `002_latest_pull_by_capture_time.sql` — recreates both views: `latest_pull` orders by `captured_at` (was ingest order — a stale-data bug found in review) and the board's rostered branch filters to the league's positions (2026-08-25); `003_superflex_display_board.sql` — the board view reads the **superflex** rankings, with defenses on positional rank only ([D-12](decision_log.md), 2026-08-26); `004_historical_data.sql` — the three historical tables (`player_season_stats`, `contract_history`, `auction_result`), loaded by `npm run ingest:historical` ([#17](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/17), 2026-08-26); `005_projection_engine.sql` — the projection layer (`projection_source` input written by `npm run ingest`; `engine_run` + `projection` derived output written by `npm run engine`) + the `latest_engine_run` view ([#18](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/18), 2026-08-26); `006_backtest_pull_isolation.sql` — adds `pull.kind` ('current' default / 'backtest') + `pull.season`, and recreates `latest_pull` (now filtered to `kind='current'`) and `board` verbatim, so the backtest's historical FantasyPros pulls (loaded by `npm run backtest`) share `market_ranking`/`projection_source` with full lineage but never surface as the live board ([#19](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/19), 2026-08-26); `007_valuation.sql` — the valuation layer (`replacement_level`, `valuation`, `price_curve`), written by `npm run engine` after the projection rows ([#20](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/20), 2026-08-26) |
