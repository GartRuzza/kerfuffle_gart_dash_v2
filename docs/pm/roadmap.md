...
| 9 | **Backtest — the decision gate ([#19](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/19))** | Does the Kerf re-rank beat raw ECR (and CBS's own projection) at predicting **actual** KERFUFFLE points on 2024 & 2025? Truth = CBS actuals (#17); prediction = historical FantasyPros ECR/projections (**confirmed accessible** via the API season param). Results viewable in the prototype. **If the edge isn't there, we fix the core before building below this line.** | #18 |
| 10 | **Valuation — VORP dollars, ceilings, price curve, Edge ([#20](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/20))** | Stage 2 ([D-13](../decision_log.md)): last-starter replacement (QB24/RB~34/WR~34/TE~17/DST12) → cap dollars → **league-generic + roster-aware ceilings**; price curve (2025 KERFUFFLE + current rosters; TRUFFLE off); **Edge = price − ceiling**; single-season value, dynasty as context; drillable. | #19 passed |
