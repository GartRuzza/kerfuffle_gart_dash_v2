...
| 4 | **League power rankings — team-strength spin-off ([#32](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/32))** | A **standalone spin-off** view (not one of the four core lenses): a dedicated screen ranking all 12 teams by roster strength — **Starter Strength** (optimal superflex lineup) + **Total Roster** (depth) + a per-position (QB/RB/WR/TE) grid + team tiers — by **aggregating the engine's per-player Kerf projected points**. Its named service is **trade scouting** (who's stacked / thin → who to trade with, for what). **Win-now/ROS, offense-only, points-based** for v1; dynasty toggle deferred ([open decision #16](#)). Reuses the engine's optimal-lineup builder — aggregation, not new modelling. | ROS engine ([#28](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/28)) |
...
| --- | --- | --- | --- | --- | --- |
| 16 | **League power rankings — v1 shape.** A team-strength spin-off ([#32](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/32)): what currency, starters vs depth, horizon, where it lives, DST, and how it fits the "every view serves a decision" principle. | The power-rankings issue ([#32](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/32)) | currency (points/dollars); starters-only/total/both; win-now/dynasty/both; dedicated screen vs table mode; DST handling; framing | Owner | **Resolved 2026-08-27 — Kerf projected points (primary); show BOTH Starter Strength + Total Roster; win-now/ROS only for v1 (dynasty toggle deferred — no KERFUFFLE dynasty projection exists); dedicated new screen; offense-only (DST excluded, no UI note); standalone spin-off serving trade scouting.** Recorded in [`../decision_log.md`](../decision_log.md) **D-20**. **Remaining open sub-question:** how to score the future dynasty toggle (dynasty-ECR proxy vs a real KERFUFFLE dynasty model) — parked until the dynasty view is scheduled. |
