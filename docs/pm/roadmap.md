...
| Automated snapshot scheduling | Snapshotting (Issue A) runs manually for now — a scheduled task with an expired CBS cookie silently collects nothing, so scheduling waits until cookie-lifetime is solved | After the auction prep lens item (Now #10) — promoted from the backlog 2026-08-24 |
...
- **Source profiling (B) must precede schema finalization (C)** — the profiler's evidence (dead-cap pseudo-rows, Practice Squad rows, transaction types, the `/rules` scoring values) is what the schema is built around.
- **Storage (C) must precede the valuation engine** — the engine reads from the normalized store and its parsed `/rules` scoring values; building it first means building it twice.
...
| 2 | FantasyPros access method | Data pipeline (#3) and everything downstream | API application vs. scrape vs. manual export (manual acceptable for MVP) | Owner, informed by spike #3 | **Resolved 2026-08-20 — official FantasyPros API on the HOF tier (~$9/mo)** (spike #7 / issue #7). Not scraping, not manual export. The API is self-serve (not approval-gated as assumed); the CBS↔FP join is a **direct `cbs_player_id` match**. Free tier is a 10-of-520 preview, so HOF is required for the full board (owner upgraded; **HOF subscription active as of 2026-08-24** — the empirical cap-lift + gated-endpoint re-verification is folded into the source-profiling spike, Now #5 / Issue B). See [`../fantasypros_data_discovery.md`](../fantasypros_data_discovery.md), [`../decision_log.md`](../decision_log.md) D-09. |
...
| 6 | Local storage architecture: files vs. a database, and how to keep history without blocking a later web deploy | Issue C (storage schema + ingestion, Now #6) and everything that reads from the store | Flat CSV/JSON vs. a real DB; chosen: three-layer **raw archive → SQLite (normalized + derived)** via `better-sqlite3`, all access behind one data module | Owner | **Decided 2026-08-24 — [`../decision_log.md`](../decision_log.md) D-10.** |
| 7 | How are dead-cap pseudo-rows and Practice Squad players represented in the schema? | Finalizing Issue C's storage schema (Now #6) — approval D-10 covers the storage *shape*, not this | Classify as non-player rows on the roster vs. a separate table vs. flags on the roster row; the Issue B profiler (Now #5) produces the evidence, owner decides | Owner, informed by Issue B | **Open — raised 2026-08-24.** |
