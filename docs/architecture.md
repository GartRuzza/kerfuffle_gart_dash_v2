...
**Last updated:** 2026-08-26 · **Reflects commit:** main. *The D-10 storage architecture's **raw archive** (issue #10), **source profiler** (issue #11), the **normalized SQLite store + ingestion + data-access module** (issue #12), the **historical-data ingestion path** (issue #17 — migration 004 + `npm run ingest:historical`), and now the **projection engine** (issue #18 — migration 005 + `npm run engine`, producing the first derived layer: Kerf points/ranks/tiers) are real. The **valuation layer** (dollars/ceilings/Edge) is still **(planned)**, gated behind the backtest.*
