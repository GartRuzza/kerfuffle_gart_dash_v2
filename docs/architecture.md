...
**Last updated:** 2026-08-26 · **Reflects commit:** main. *The D-10 storage architecture's **raw archive** (issue #10), **source profiler** (issue #11), the **normalized SQLite store + ingestion + data-access module** (issue #12), the **historical-data ingestion path** (issue #17 — migration 004 + `npm run ingest:historical`), the **projection engine** (issue #18 — migration 005, Kerf points/ranks/tiers), the **backtest gate** (issue #19 — migration 006 + `npm run backtest`), and now the **valuation engine** (issue #20 — migration 007, folded into `npm run engine`: VORP dollars, two ceilings, price curves, Edge) are all real. **The dollar columns now show real numbers** (offense only). The MVP's engine is complete; the remaining Now item is the auction-prep lens (persisted ceilings + cap-sum check, roadmap #10).*
