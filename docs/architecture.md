...
**Last updated:** 2026-08-26 · **Reflects commit:** main. *The D-10 storage architecture's **raw archive** (issue #10), **source profiler** (issue #11), the **normalized SQLite store + ingestion + data-access module** (issue #12), the **historical-data ingestion path** (issue #17 — migration 004 + `npm run ingest:historical`), the **projection engine** (issue #18 — migration 005 + `npm run engine`, producing the first derived layer: Kerf points/ranks/tiers), and now the **backtest gate** (issue #19 — migration 006 + `npm run backtest`, re-running the core on isolated historical pulls to measure the edge over ECR) are real. The **valuation layer** (dollars/ceilings/Edge) is still **(planned)**, awaiting the owner's go/no-go on the backtest's marginal edge.*
...
Still **(planned)**, growing inside this same app: the **valuation layer** (dollars/ceilings/price curve/Edge — issue [#20](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/20), gated behind the backtest [#19](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/19)), and the auction/waiver/trade lenses — all reusing this one table.
...
| Data source | **Built (issue #12; extended #18)** — the data-access module `lib/data/` (`board.ts` + pure `derive.ts`) reading the SQLite `board` view + the latest `engine_run`'s `projection` rows, read-only | THE read boundary (D-10): returns the `Player` shape from the latest ingested pull; free agents derived from the FantasyPros board; **Kerf ranks/tiers + Proj Points come from the latest engine run** (issue #18), dollar fields null until the valuation issue. Replaced `lib/mockData.ts` | [D-10](decision_log.md) |
...
- **The projection engine is a separate, offline operator process — not part of the app (built, issue #18).** `tools/engine/run.mjs` (`npm run engine`) reads **only** the local store (never the network), computes Kerf points/ranks/tiers inside one transaction, and writes the derived `engine_run` + `projection` rows. Its **pure core** (`tools/engine/core.mjs`) holds all the math (first-down rates, estimation, scoring, ranks, Jenks tiers) with no DB/filesystem, so it is unit-tested in isolation and reproduces deterministically. The app reads the result through `lib/data/` like any other stored data — **no engine math runs at request time**; if the engine has never run, Kerf fields simply render "—".
...
| Kerf **dollar** columns (Kerf Value, Market Value, Edge, Ceiling seed) still blank ("—") | Every day until the valuation layer (#20) lands — deliberately gated behind the backtest (#19) | The valuation issue: `replacement_level`/`valuation`/`price_curve` on top of the projection layer that now exists |
