...
Still **(planned)**, growing inside this same app: a **historical-data ingestion path** (issue [#17](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/17) — reads owner-provided CSVs from `data/historical/` into new tables, on a path separate from the archiver), the **valuation engine** (a pure module + the derived DB layer, staged across issues [#18](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/18)/[#20](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/20) with the backtest [#19](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/19) as the gate), and the auction/waiver/trade lenses — all reusing this one table. The engine reads FantasyPros component projections (current season) and historical FantasyPros ECR/projections (confirmed reachable via the API's season param) at ingest time, never at request time.
