...
**Last updated:** 2026-08-26 · **Reflects commit:** docs/valuation-engine-plan. *The D-10 storage architecture's **raw archive** (issue #10), **source profiler** (issue #11), the **normalized SQLite store + ingestion + data-access module** (issue #12), and now the **historical-data ingestion path** (issue #17 — migration 004 + `npm run ingest:historical`) are real. The **derived layer** (engine outputs) is still **(planned)**.*
...
Still **(planned)**, growing inside this same app: the **valuation engine** (a pure module + the derived DB layer, staged across issues [#18](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/18)/[#20](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/20) with the backtest [#19](https://github.com/GartRuzza/kerfuffle_gart_dash_v2/issues/19) as the gate), and the auction/waiver/trade lenses — all reusing this one table. The engine reads FantasyPros component projections (current season) and historical FantasyPros ECR/projections (confirmed reachable via the API's season param) at ingest time, never at request time.
